...
- `openmcp-cli web` now requires a subcommand and only shows help without one.
- Available subcommands: `run`, `start`, `status`, `stop`.
- `web` is decoupled from `gateway` (no auto start/stop).
...
- `web status -p` and `web stop -p` support explicit Web UI port.
